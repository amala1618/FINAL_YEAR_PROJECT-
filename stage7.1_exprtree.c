int local_binding_stored_here=-3;
int get_index=0;
int activ_ate=0;
...
void createLsymbolEntries(struct Paramstruct *paramlist)
{
	struct Paramstruct *temp;
	temp=paramlist;
        printf("HI in createLsymbol Entries");
	int x=0;
	...
	{       
		...
		temp=temp->next;
		local_binding_stored_here=local_binding_stored_here-1;
...
void print_Typetable()
{
	...
	while(temp!=NULL)
	{
		ftemp=temp->fields;
		if(ftemp==NULL){
			printf("Name is:%s\t\tSize is :%d\t\tFields is NULL\t\n",temp->name,temp->size);
		        printf("-------------------------------------------------------\n");}

		else
		{
			printf("Name is :%s\t\tSize is :%d\t\tFields is below\t\n",temp->name,temp->size);
			printf("FIELDLIST OF %s\n",temp->name);
			while(ftemp!=NULL)
			{
				
				printf("Field Index is:%d\t\tName of Field Type:%s\t\tName of Field:%s\t\n",ftemp->fieldIndex,ftemp->type->name,ftemp->name);
				ftemp=ftemp->next;
			}
			printf("-------------------------------------------------------\n");
...
void func_check(char *varn,struct tnode *left){
	...
	while(left!=NULL){
		...
		if(def_param_list==NULL){
			...
			printf("While Calling the function, Parameter Type Mistmatch at Argument %d of Function %s\n",count,varn);
	...
	if(def_param_list!=NULL)
	{
		printf("Few arguments are missing in the Function call for %s\n",varn);
		exit(1);
	}
}
void fucn_check_name(struct Typetable *rettype,char *name,struct Paramstruct *paramlist)
{
	...
	if(dummy==NULL)
	{
		printf("Function defnition %s without DECLARATION!!\n",name);exit(1);	}
	if(dummy->type!=rettype)
	{	printf("Return type \"%s\" MISMATCH!!\n",name);	/*exit(1);*/	}
	...
	{       count++;
		...
		{
			printf("Conflict in TYPE/NAME at Parameter %d of Function %s\n",count,name);exit(1);}
		else
		{
			plist=plist->next;temp=temp->next;	}
	}
	if(temp!=NULL)
	{
...
int Loc_In_Reg(struct tnode* t, FILE *fptr){
	...
	if(t->Lentry!=NULL){
		switch(t->nodetype){
			...
			case 30://selfnode
				  {       //fprintf(fptr,"Its a problem with get adder\n");
					  ...
					  fprintf(fptr, "ADD R%d, %d\n",reg,loc);
					  t = t;
					  //if(t->right==NULL){fprintf(fptr,"RIGHT IS NULL!!!!!!!!");}
...
int codeGen(struct tnode *t, FILE *target_file) {
		...
		case 20 : p=getReg();
			  ...
			  fprintf(target_file,"MOV R%d,[%d]",p,t->left->val);
                ...
                case 21 ://&ID
		      	  printf("Node type is %d",t->left->nodetype);
		...
		case 30: //self.length type
			...
			fprintf(target_file, "MOV R%d, [R%d]\n", reg_1, reg_1);
			return reg_1;
		case 31://fprintf(target_file,"Saving Machine state\n");
			  temp_count=-1;
			  for(n=0;n<=get_index;n++){
				   fprintf(target_file,"PUSH R%d\n",n);
				  temp_count=temp_count+1; 
			   ...
			   //fprintf(target_file,"Evaluating Arguments and Pushing to stack\n");
			   x=Loc_In_Reg(t->left,target_file);
			   fprintf(target_file,"PUSH R%d\n",x);
			   freeReg();
			   arglist=t->next;
			   while(arglist!=NULL)
