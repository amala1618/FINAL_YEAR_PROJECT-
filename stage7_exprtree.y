%{
	...
	#include <stdio.h>
        #include <string.h>
	#include "stage7_exprtree.h"
	#include "stage7_exprtree.c"
	int yylex(void);
        FILE* fp;
        FILE* yyin;
        extern int yylineno;
...
FieldDeclList : FieldDeclList FieldDecl {$2->next = $1;$$ = $2; Fhead=$2; FieldNo++;if(FieldNo>=8){printf("Number of fields cannot exceed 8\n");exit(1);}}
	      | FieldDecl                {$$=$1;}
              ;
...
Fdef :Type ID '(' ParamList ')''{' LdeclBlock Body '}' {/*fucn_check_name($1,$2->varname,$4);*/codeGen_f($8,$2->varname,fp);Lsymbol_rst();activate=0;}
     |Type ID '('ParamList')''{'Body'}'                {/*fucn_check_name($1,$2->varname,$4);*/codeGen_f($7,$2->varname,fp);Lsymbol_rst();activate=0;}
...
LdeclBlock :DECL LDecList ENDDECL {/*printf("Hi in local declaration");*/activate=1;if(Cptr!=NULL){printf("CPTR not null,so installing self");LInstall("self",TLookup("VOID"));}}
	   | DECL ENDDECL         {activate=1;if(Cptr!=NULL){printf("CPTR not null,so installing self");LInstall("self",TLookup("VOID"));}}
           ;
...
FieldFunction : SELF '.' ID '(' ArgList ')'  {$$=createTree(0,TLookup("NULL"),NULL,28,NULL,NULL,NULL);}
	      |ID '.' ID '(' ArgList ')'   {$$=createTree(0,TLookup("NULL"),NULL,28,NULL,NULL,NULL);}
